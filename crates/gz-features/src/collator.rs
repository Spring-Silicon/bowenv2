...
impl FeatureCollator {
    ...
    pub fn collate_into(&mut self, rows: &[FeatureRow], out: &mut Vec<u8>) -> FeatureResult<()> {
        self.collate(rows, out)
    }

    fn collate(&mut self, rows: &[FeatureRow], out: &mut Vec<u8>) -> FeatureResult<()> {
        if rows.is_empty() {
...
impl BatchLayout {
    ...
    fn from_dims(b: usize, n: usize, e: usize, a: usize, s: usize, d: usize) -> Self {
        ...
        // Retain the removed trajectory-id/row region so batch-v2 offsets stay stable.
        section(&mut cursor, b * 12);
