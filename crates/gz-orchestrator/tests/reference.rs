...
#[test]
fn shared_gated_registry_claims_one_rollout_per_version() {
    let mut engine = whittle();
    let root = engine.root();
    let v1 = ModelVersion::from_bytes([1; 16]);
    let v2 = ModelVersion::from_bytes([2; 16]);
    let registry = Arc::new(gz_measurer::ReferenceRegistry::new());
    let mut first = PolicyReferenceProvider::gated_with_registry(Arc::clone(&registry));
    let registry_view = Arc::clone(&registry);
    let mut second = PolicyReferenceProvider::gated_with_registry(registry);
    ...
    assert!(Arc::ptr_eq(
        &reference.steps,
        &registry_view.current().unwrap().steps
    ));
